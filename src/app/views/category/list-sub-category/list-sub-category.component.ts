...
export class ListSubCategoryComponent implements OnInit {
  ...
  async getPageData(page_no){
    ...
    this._categoryService.subCategorylist({page_no}).subscribe(res => {
      MAIN.spinner.hide();
      if(res && res.code == 200){
  ...
  async deleteSubCategory(category_id) {
    const modalRef = this.modalService.open(DeleteRecordComponent, {
      size: 'md',
      backdrop: 'static'
    });
    modalRef.componentInstance.id = category_id;
    modalRef.componentInstance.msg = 'Are You Realy Want to Delete This Sub Category?';
    modalRef.componentInstance.actionFor = 'subcategory';
